{
  ...
  inherit (lib.typrs) boolean;

  cfg = config.rysieko.matugen;
in {
  rysieko.matugen = {
    enable = mkEnableOption "enable Matugen theming (templates hardcoded fuck you)";
  ...
  environment.systemPackages = mkIf cfg [
    pkgs.matugen
  ];
  cfg.targets = mkIf cfg.autoEnable {
    gtk = true;
    qt = true;
    waybar = true;
    hyprland = true;
